...
class ReleaseDocumentationTest(unittest.TestCase):
    def test_release_version_is_consistent(self):
        self.assertIn('version = "1.16.1"', Path("pyproject.toml").read_text())
        self.assertIn("v1.16.1", Path("README.md").read_text())
        self.assertIn("v1.16.1", Path("src/job_hunter/__init__.py").read_text())
