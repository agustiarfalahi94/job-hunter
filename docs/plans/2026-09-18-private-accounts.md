...
- Local full suite passes; PostgreSQL functional tests skip on this Mac because server tools are absent. CI requires those tests and installs disposable database tools. Feature/main CI and browser results are recorded in the release handoff after they run.
...
- [ ] Write tests for disabled defaults, incomplete enabled setup, secret redaction, trusted HTTPS Supabase endpoint, Google-only metadata/issuer, verified-email allowlist, stable ownership and expired identity.
- [ ] Run the new tests and confirm missing functionality fails.
- [ ] Implement strict validation and safe fixed error messages. Use repr=False for secret fields.
- [ ] Run identity/config regressions before moving to storage.
...
- [ ] Write failing round-trip tests for CV and manually recorded applications, retained custom criteria/mode, duplicate rediscovery preserving Applied, empty state, owner mismatch, unknown schema, malformed records and size limits.
- [ ] Write failing transport tests for encryption, bounded timeout, no redirects, secret-key header, owner-scoped RPC, invalid replies, sanitized errors and revision conflicts.
- [ ] Implement authenticated Fernet snapshots using a deployment-only encryption key; never persist caches or controllers.
- [ ] Implement service-only SQL RPCs. Initial insert is conflict-safe; updates and clear require the current revision. Clear increments revision and nulls payload, preserving a tombstone.
- [ ] Validate migration syntax, grants, anonymous denial, CAS and tombstones against a local disposable Postgres if available. Document any external tests not performed.
...
- [ ] Write failing tests for account round trips after simulated restart; failed load cannot save; unchanged reruns cannot write; conflict cannot overwrite; explicit retry/reload; delete confirmation; logout cancels workers and clears private state; disabled accounts preserve guest behavior.
- [ ] Show Google login only with complete enabled setup. Denied identities cannot reach the workspace/backend. No guest bypass when private accounts are enabled.
- [ ] Restore workspace/settings/mode once per identity. Save settings callbacks, CV save/remove, posting/application edits, and accepted background results. Show actual Saved/Unsaved/conflict/error status.
- [ ] Add explicit reload/retry and confirmed account-data deletion. Do not silently discard unsaved changes or continue a stale controller after logout/reload.
- [ ] Add a private-storage notice before account CV upload. Keep Profile & CV hidden in criteria mode.
- [ ] Verify the app imports and navigation in disabled, signed-out, denied and signed-in mocked states.
...
- [ ] Add Authlib and cryptography dependency floors, bump to 1.18.0, and document backend encryption-key backup/retention limits and recovery.
- [ ] Provide exact Google callback, Supabase SQL migration, secret-key type and private enablement checklist. Provide local secret-generation commands for owner use; never run them into chat/logs or commit their output.
- [ ] Run the full check script, compileall, dependency consistency and diff checks; obtain independent security/correctness review and fix findings.
