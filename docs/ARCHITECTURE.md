...
Web search selections live in a non-widget `search_settings` dictionary. Disposable underscore-prefixed widget keys copy changes into it using callbacks, and render from saved values. This prevents Streamlit's off-page widget cleanup from resetting criteria or queue hard-skip filtering. Criteria-mode sessions select nothing by default. In v1.18.4, CV mode computes a digest of readable CV text and performs one case-insensitive merge of configured roles/hard skips plus detected primary/bonus skills. The digest is saved in the encrypted account snapshot so later manual edits are not reapplied after reboot. Location and sources are never inferred from CV text.
...
v1.18.5 retains v1.18.3 hosted pagination: provider-advertised API pages follow initial source/signal queries. When title and description signals are both empty, the planner produces one broad signal pass for selected sources instead of multiplying empty title/description passes; direct LinkedIn may still expand selected location scopes inside the request ceiling. Exact official SerpAPI links with strictly advancing offsets up to 110 are preferred pagination evidence. Standard Google responses may use a bounded manual offset only after a full ten-result page where the pagination field is entirely absent, up to offset forty. A present pagination field without a valid next link, a short page, structured Jobs results, repeated results, or an empty page stops. The server reconstructs its original query/key/filter parameters and never follows the response destination. Later pages share the twelve-request and fifty-candidate limits. Parsing Indeed titles works from recognized trailing location/site components, preserving internal role separators and leaving unverified employers unknown.
